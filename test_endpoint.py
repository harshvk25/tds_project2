#!/usr/bin/env python3
import requests, os, sys, json
from datetime import datetime

ENDPOINT_URL = os.environ.get("ENDPOINT_URL", "http://localhost:5000/quiz")
EMAIL = os.environ.get("STUDENT_EMAIL")
SECRET = os.environ.get("STUDENT_SECRET")

def test_health(base_url):
    try:
        r = requests.get(base_url.replace('/quiz','/health'), timeout=5)
        assert r.status_code == 200
        print("✅ Health check passed")
        return True
    except: print("❌ Health check failed"); return False

def test_invalid_json(url):
    try:
        r = requests.post(url, data="not json", headers={"Content-Type":"application/json"}, timeout=5)
        assert r.status_code == 400
        print("✅ Invalid JSON test passed")
        return True
    except: print("❌ Invalid JSON test failed"); return False

def test_invalid_secret(url):
    try:
        r = requests.post(url, json={"email":EMAIL, "secret":"wrong","url":"https://example.com"}, timeout=5)
        assert r.status_code == 403
        print("✅ Invalid secret test passed")
        return True
    except: print("❌ Invalid secret test failed"); return False

def test_demo_quiz(url):
    try:
        print("⏳ Testing demo quiz (may take 30-60s)...")
        r = requests.post(url, json={"email":EMAIL,"secret":SECRET,"url":"https://tds-llm-analysis.s-anand.net/demo"}, timeout=180)
        assert r.status_code == 200
        print("✅ Demo quiz test passed")
        print(json.dumps(r.json(), indent=2))
        return True
    except: print("❌ Demo quiz failed"); return False

def run_tests():
    print(f"Testing endpoint: {ENDPOINT_URL} at {datetime.now()}")
    results = [
        test_health(ENDPOINT_URL),
        test_invalid_json(ENDPOINT_URL),
        test_invalid_secret(ENDPOINT_URL),
        test_demo_quiz(ENDPOINT_URL)
    ]
    print(f"\n✅ Passed {sum(results)}/{len(results)} tests")
    return 0 if all(results) else 1

if __name__ == "__main__":
    if not EMAIL or not SECRET:
        print("❌ Set STUDENT_EMAIL and STUDENT_SECRET"); sys.exit(1)
    sys.exit(run_tests())
